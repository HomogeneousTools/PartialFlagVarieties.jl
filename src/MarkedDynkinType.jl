...
subgroup ``P``. In the convention used by this package, marked nodes are
the *crossed-out* nodes of the Dynkin diagram: the parabolic ``P_I`` is
generated by ``B`` and ``U_{-\\alpha_j}`` for ``j \\notin I``.
"""
...
``[L, L]``. These are the *uncrossed* nodes.
"""
...
Return the rank of the semisimple Levi factor ``[L, L]``, i.e. the number
of unmarked nodes.
"""
