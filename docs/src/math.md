...
The parabolic subgroup ``\mathrm{P} = L \ltimes U`` has a **Levi decomposition** where:
...
- If ``\mu`` is **regular**, there exists a unique ``w \in W`` such that
  ...
  - ``\mathrm{H}^{\ell(w)}(\mathrm{G}/\mathrm{B}, \mathcal{L}_\lambda) \cong V_{w(\mu) - \rho}`` as ``G``-modules
...
- **Dimension**: ``\dim(\mathrm{G}/\mathrm{P}) = |\Phi^+_G| - |\Phi^+_L|`` (positive roots of ``G``
  minus positive roots of the Levi)
- **Euler characteristic**: ``\chi(\mathrm{G}/\mathrm{P}) = |\mathrm{W}_G| / |\mathrm{W}_L|``
